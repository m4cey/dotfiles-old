    ...
    class FAD extends react.Component {
        ...
        async fetchInfo() {
            ...
            try {
                ...
                /*if (meta.is_local) {
                    const uriFinal = meta.local_file_path.split("/");
                    uriFinal.pop();
                    finImage = 'file://' + uriFinal.join("/") + '/cover.jpg';
                }*/
                const artist = encodeURI(meta.artist_name);
                const album = encodeURI(meta.album_title);
                const ximage = await Spicetify.CosmosAsync.get('http://ws.audioscrobbler.com/2.0/?method=album.getinfo&api_key=47475241a4bfa5531c95bd0ee8d18f22&artist=' + artist + '&album='+ album + '&format=json')
                finImage =  ximage.album.image[4]["#text"];
            }
            // prepare title
